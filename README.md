...
- **파일 미리보기**: 추가 서류 업로드 시 이미지 실시간 미리보기 지원
- **파트너 네트워크 추가 서류 업로드**: 매입거래처, 프랜차이즈본사/지점, 인테리어, 요식업, 기타 카테고리에서도 추가 서류 업로드/미리보기/다운로드 가능
...
- **📎 회사 사업자등록증 관리**: 
  ...
  - Base64 인코딩하여 DB 저장
- **📂 추가 서류 관리**:
  - **적용 카테고리**: 외주팀 관리, 매입거래처, 프랜차이즈본사, 프랜차이즈지점, 인테리어, 요식업, 기타
  - 통장사본, 명함, 계약서 등 여러 파일 업로드
  - 이미지 파일: 실시간 미리보기 지원
  - PDF 파일: 아이콘과 파일명 표시
  - 파일별 개별 다운로드/삭제 기능
  - 최대 10MB 파일 지원
...
### 2026-01-25 (최신) - 파트너 네트워크 수정 모달 파일 업로드 UI 수정 완료 🎯

#### ✅ 파트너 네트워크 수정 모달 파일 업로드 UI 표시 문제 해결
- **문제**: 
  - 파트너 네트워크 카테고리(매입거래처, 프랜차이즈본사/지점, 인테리어, 요식업, 기타) 수정 모달에서 "📄 회사 사업자등록증" 섹션이 표시되지 않았음
  - 신규 등록 시에는 표시되지만, 수정 모달 열 때는 사라지는 문제
  
- **해결**: 
  - ✅ **ContactFormModal에 useEffect 추가** - `initialData.licenseFile` 자동 미리보기 설정
  - ✅ **회사 사업자등록증 섹션 추가** - 파트너 네트워크 카테고리의 "다른 카테고리 UI" 블록에 사업자등록증 업로드 섹션 추가
  - ✅ **수정 모드 파일 미리보기** - 기존 업로드한 파일 자동 표시
  - ✅ **파일 다운로드** - 수정 모드에서도 기존 파일 다운로드 가능
  - ✅ **파일 재선택/삭제** - 기존 파일 교체 및 삭제 기능
  - ✅ **신규/수정 모두 동일 UI** - 신규 등록과 수정 모드에서 동일한 파일 업로드 UI 표시

- **적용 카테고리** (6개):
  - 매입 거래처 (PURCHASE)
  - 프랜차이즈 본사 (FRANCHISE_HQ)
  - 프랜차이즈 지점 (FRANCHISE_BR)
  - 인테리어 (INTERIOR)
  - 요식업 (SALES)
  - 기타 거래처 (OTHERS)
...
- **배포 완료**:
  - ✅ Cloudflare Pages 프로덕션 배포: https://f8e14437.gs-connect.pages.dev
  - ✅ Git 커밋: `Fix: Add file upload UI to partner network edit modal`
  - ✅ 배포 시간: 2026-01-25 06:13

### 2026-01-21 - 파트너 네트워크 파일 업로드 기능 완성 🎯

#### ✅ CompanyModal 수정 모드 파일 미리보기 추가
- **문제**: 회사 수정 모달에서 기존 업로드한 파일이 보이지 않는 문제
- **해결**: 
  - ✅ **수정 모달 열릴 때 사업자등록증 자동 미리보기** - initialData.licenseFile 자동 로드
  - ✅ **기존 파일 다운로드 버튼 활성화** - 수정 모드에서도 다운로드 가능
  - ✅ **이미지 파일 미리보기** - 사업자등록증 이미지 즉시 확인
  - ✅ **PDF 파일 아이콘 표시** - PDF 문서는 아이콘으로 표시
  - ✅ **파일 재선택/삭제** - 기존 파일 교체 및 삭제 가능

- **기술 구현**:
  - `useEffect` 추가하여 `initialData.licenseFile` 자동 미리보기 설정
  - 다운로드 버튼 조건을 `formData.licenseFile || initialData?.licenseFile`로 수정
  - 이미지/PDF 분기 처리하여 적절한 미리보기 표시

#### ✅ 6개 카테고리 추가 서류 업로드 지원 (ContactFormModal)
- **적용 카테고리**: 
  - 매입 거래처
  - 프랜차이즈 본사
  - 프랜차이즈 지점
...
- **신규 기능**:
  - ✅ **수정 모달 열릴 때 기존 파일 자동 표시** - 업로드한 사진 즉시 확인 가능
  - ✅ **파일 추가 UI** - "📎 추가 서류" 섹션 추가
  - ✅ **이미지 실시간 미리보기** - JPG, PNG 등 이미지 파일 미리보기
  - ✅ **PDF 아이콘 표시** - PDF 문서 아이콘으로 표시
  - ✅ **파일별 다운로드** - 각 파일마다 개별 다운로드 버튼
  - ✅ **파일별 삭제** - 불필요한 파일 개별 삭제
  - ✅ **여러 파일 동시 선택** - 한 번에 여러 파일 업로드 가능
  - ✅ **Base64 DB 저장** - 파일 데이터 안전하게 저장

- **기술 구현**:
  - ContactFormModal에 additionalFilesInputRef 및 isUploadingAdditional state 추가
  - formData.attachments 배열로 파일 관리
  - isPartnerNetwork 조건으로 UI 표시 제어
  - 통장사본, 명함, 계약서 등 모든 서류 첨부 가능

### 2026-01-13 - GitHub Actions 자동 배포 시스템 구축 🚀
...
- ✅ **파트너 네트워크 수정 모달 파일 업로드 UI 수정** (2026-01-25)
- ✅ **파트너 네트워크 추가 서류 업로드** (매입/프랜차이즈/인테리어/요식업/기타)
